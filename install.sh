#!/usr/bin/env sh
# install.sh — download the latest alegra release into a local bin dir.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/jjuanrivvera/alegra-cli/main/install.sh | sh
#
# Environment:
#   ALEGRA_VERSION       pin a version, e.g. "v0.9.4" (default: latest)
#   ALEGRA_INSTALL_DIR   install location (default: $HOME/.local/bin)
#   NO_COLOR             disable colored output

set -eu

# --- configuration (per project) ---------------------------------------------
REPO="jjuanrivvera/alegra-cli"         # GitHub <owner>/<repo>
PROJECT="alegra-cli"                   # release asset prefix (goreleaser project_name)
BINARY="alegra"                        # installed binary (name inside the archive)
VERSION_IN_NAME=1                      # 1 if the asset filename embeds the version
ARCH_AMD64="amd64"                     # token used for x86_64 hosts in asset names
VERSION="${ALEGRA_VERSION:-latest}"
INSTALL_DIR="${ALEGRA_INSTALL_DIR:-$HOME/.local/bin}"
# ------------------------------------------------------------------------------
