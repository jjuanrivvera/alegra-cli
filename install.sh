#!/bin/sh
# alegra installer for macOS and Linux.
#
...
#   ALEGRA_VERSION=v1.2.3   pin a version (default: the latest release)
...
REPO="jjuanrivvera/alegra-cli"
BINARY="alegra"
VERSION="${ALEGRA_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set ALEGRA_VERSION"
fi
